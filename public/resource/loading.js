...
try {
  ...
  // browser navigation bar theme color (mobile)
  if (metaThemeColor) {
...
function initLoadingPage(selector = appSelector) {
  ...
  if (appElement) {
    appElement.innerHTML = `<img class="app-loading-page-logo-svg" src="${appLogoData}" width="120" height="120" alt="logo" />`
    document.body.appendChild(appElement)
    document.documentElement.classList.add('app-page-loading')
  }
}
initLoadingPage()

window.removeLoadingPage = function () {
  const el = document.querySelector(appSelector)
  if (!el)
    return
  el.classList.add('app-loading-page-wrapper--hiding')
  el.addEventListener('transitionend', () => {
    el.remove()
    document.documentElement.classList.remove('app-page-loading')
  }, { once: true })
}
